...
public class SearchResultsServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if(request.getSession().getAttribute("select").equals("galaxies")){
			request.getRequestDispatcher("/WEB-INF/GalaxyResults.jsp").forward(request, response);
		}
		else if(request.getSession().getAttribute("select").equals("solar_systems")){
			request.getRequestDispatcher("/WEB-INF/SolarSystemResults.jsp").forward(request, response);
		}
		else if(request.getSession().getAttribute("select").equals("stars")){
			request.getRequestDispatcher("/WEB-INF/StarResults.jsp").forward(request, response);
		}
		else if(request.getSession().getAttribute("select").equals("Small_Solar_System_Bodies")){
			request.getRequestDispatcher("/WEB-INF/SmallSolarSystemBodyResults.jsp").forward(request, response);
		}
		else if(request.getSession().getAttribute("select").equals("planets")){
			request.getRequestDispatcher("/WEB-INF/PlanetResults.jsp").forward(request, response);
		}
		else if(request.getSession().getAttribute("select").equals("blackholes")){
			request.getRequestDispatcher("/WEB-INF/BlackHoleResults.jsp").forward(request, response);
		}
		else if(request.getSession().getAttribute("select").equals("moons")){
			request.getRequestDispatcher("/WEB-INF/MoonResults.jsp").forward(request, response);
		}
	}

	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("got here with" + request.getParameter("select") + " " + request.getParameter("query"));
		String select = request.getParameter("select");
		request.getSession().setAttribute("select", select);
		String query = request.getParameter("query");
		Object object = getQuery(select, query);
		request.getSession().setAttribute("object", object.getClass().cast(object));
	...
	public Object getQuery(String search, String query){
		try {
			Class.forName("org.firebirdsql.jdbc.FBDriver");
			 ...
			 ResultSet rs = connection.prepareStatement("select * from " + search + " where name = " + "\'" + query + "\'").executeQuery();
			 while(rs.next()){
				 if(search.equals("galaxies")){
					 ...
					 ResultSet orbitSet = connection.prepareStatement("select g.name from galaxies g"
					 		+ "											inner join solar_systems s"
					 		+ "											on(g.id = s.galaxy_id)"
					 		+ "											where s.id = " + ss.getId()).executeQuery();
					 orbitSet.next();
					 ...
					 ResultSet orbitSet = connection.prepareStatement("select s.name from solar_systems s"
						 		+ "											inner join stars ss"
						 		+ "											on(s.id = ss.solar_system_id)"
						 		+ "											where ss.id = " + star.getId()).executeQuery();
						 orbitSet.next();
					 ...
					 ResultSet orbitSet = connection.prepareStatement("select s.name from stars s"
						 		+ "											inner join small_solar_system_bodies ss"
						 		+ "											on(s.id = ss.star_id)"
						 		+ "											where ss.id = " + s.getId()).executeQuery();
						 orbitSet.next();
					 ...
					 ResultSet habitableSet = connection.prepareStatement("select habitable from planets_habitable"
					 		+ "											where type = \'" + planet.getType() + "\'").executeQuery();
						 habitableSet.next();
				     planet.setHabitable(habitableSet.getString(1));
					 ResultSet orbitSet = connection.prepareStatement("select s.name from stars s"
						 		+ "											inner join planets p"
						 		+ "											on(s.id = p.star_id)"
						 		+ "											where p.id = " + planet.getId()).executeQuery();
						 orbitSet.next();
					 ...
					 ResultSet orbitSet = connection.prepareStatement("select p.name from planets p"
						 		+ "											inner join moons m"
						 		+ "											on(p.id = m.planet_id)"
						 		+ "											where m.id = " + moon.getId()).executeQuery();
						 orbitSet.next();
